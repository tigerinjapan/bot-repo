...
def get_today_news_image(forecast: str, today_outfit: str, msg: str) -> str:
    ...
    contents = (
        "480px X 360pxのサイズに合わせて、ニュース番組の大きいモニター画面イメージを生成してください。"
        "以下内容を参考してください。"
        "1. 画面イメージには、日本語、中国語、記号など英語以外の文字は、一切表示しない。"
        "2. モニター画面は、400px X 300pxのサイズ、背景色が真っ白で、空白で、何も表示されていない。"
        "3. モニター画面は、イメージの中央に位置し、テレビスタンドがない。"
        "4. 番組名は、「Today's News」、画面の左上に表示する。英語の番組名だけ、表示する。"
        f"5. イメージのの背景は、{forecast}が分かるようにする。"
        "6. その背景は、暗い色や複雑なイメージは避ける。"
        f"7. 天気キャスターの{img_div}が、全体イメージの中で、右側にいる。"
        f"8. {img_div}は、本物と同じ自然なイメージにする。イラストではない。"
        f"9. {img_div}は、正面を見ながら、笑顔でいる。"
        f"10. {img_div}は、40才の日本人、身長は155cm、50kg、小柄、童顔、笑顔、可愛い人。"
        f"11. {img_div}の今日のファッションは、{today_outfit}。"
    )
