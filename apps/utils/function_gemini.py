...
def get_today_news_image(forecast: str, today_outfit: str, msg: str) -> str:
    ...
    contents = (
        "空白で何も表示されていない、ニュース番組の大きいモニター画面がある。"
        "番組名は、「Today's News」、画面の左上に表示する。番組名以外の文字は、不要。"
        f"背景は、{forecast}が分かるようにする。"
        "背景は、暗い色や複雑なイメージは避ける。"
        f"天気キャスターの{img_div}が、全体イメージの中で、右側にいる。"
        f"{img_div}は、本当の人と同じ自然なイメージにする。イラストではない。"
        f"{img_div}は、40才の日本人、身長は155cm、50kg、小柄、童顔、笑顔、可愛い人。"
        f"{img_div}の今日のファッションは、{today_outfit}。"
        "480ピクセル X 360ピクセルのサイズに合わせて、イメージを生成。"
        "イメージ全体には、日本語、中国語、記号など英語以外の文字は、一切表示しない。"
    )
