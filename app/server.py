...
from utils.message_constants import (MSG_ERR_PASSWORD_INCORRECT,
                                     MSG_ERR_USER_NOT_EXIST, MSG_INFO_LOGOUT)
...
async def login(
    ...
    if user and user["userId"] == userId and user["userPassword"] == userPassword:
        ...
        chk_msg = MSG_ERR_PASSWORD_INCORRECT
        if not user or user["userId"] != userId:
            chk_msg = MSG_ERR_USER_NOT_EXIST
        request.session.clear()
...
async def logout(request: Request):
    request.session.clear()
    context = {const.STR_REQUEST: request, "chk_msg": MSG_INFO_LOGOUT}
